...
class RequestIDMiddleware(MiddlewareMixin):
    ...
    def __call__(self, request):
        ...
        if hasattr(response, F.CONTENT) and hasattr(response, F.CONTENT_TYPE_FOR_REPR):
            if response.content and F.APPLICATION_JSON in response._content_type_for_repr:
